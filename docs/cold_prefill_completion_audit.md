...
| Patch 06 safe cold-zero conv fast path | `safe_cold_zero_conv_fast_path`; zero-prefix and hybrid-restore guard tests | Done locally |
...
| Short baseline reproduction gate | Strict-final acceptance checks the documented ~420 tok/s baseline on each fixed short baseline prompt, not only a global aggregate | Done locally |
...
| Command | Result |
| --- | --- |
| Focused unit suite for serving config, cold-prefill guards, benchmark, acceptance, strict-final orchestration, proxy/server metrics, and hybrid validation metrics | Passed, 153 tests |
...
| `python3 -m pytest contrib/models/Qwen3.6-27B/test/unit` | Passed locally: 180 passed, 4 skipped because `neuronx_distributed_inference` is not installed |
...
| --- | --- |
| Baseline reproduction | `A_single512_old_chunked` rows near documented `420 tok/s` with same prompt set, `max_model_len`, and compiled artifacts; fixed short baseline prompts are checked individually |
...
| Hybrid APC exactness | `qwen36_hybrid_apc_validation.py exactness` report with full-prefix and partial-prefix token exactness, `cold_full`/`cold_partial` cold-prefill metrics, and strict cold-zero/chunked/text-only/compact validation config |
...
| Feature deltas | A->G deltas for buckets, text-only CTE, compact mask, fused GDN, cold-zero conv, and tile/block sweep do not regress in latency, tok/s, token exactness, or HBM when HBM is present |
