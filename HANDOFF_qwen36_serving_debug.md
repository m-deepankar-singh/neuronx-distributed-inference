...
66. Compile monitor prompt now names next-compile preflight helper.
   - Updated `validation_scripts/qwen36_compile_monitor_prompt.py` so future heartbeat monitors no longer describe the old manual "new dry-run env log + prompt generator" path. When `speed_slice_decision.json` says `launch_next_speed_slice`, the monitor prompt now instructs: run `qwen36_next_compile_preflight.py --decision-json ... --output-json ... --automation-json-output ...`, create the fresh heartbeat automation from `<automation-payload.json>` with `codex_app.automation_update`, and only then run `compile_command_after_automation` from `<next-compile-preflight.json>`.
