...
- Updated `validation_scripts/qwen36_compile_monitor_prompt.py` so future compile automations must copy the compile env log to the runtime host, run a `LAUNCH_DRY_RUN=1` launch, audit it with `qwen36_launch_env_audit.py`, and only then start live vLLM. This closes the gap where a runtime validator could be pointed at a compile-host-only env log path or launched with widened 256k buckets against a 32k artifact.
- Added `validation_scripts/qwen36_launch_preflight.py` so monitors can derive the dry-run launch environment directly from the compile env log, run the launch dry run and audit, and then use the emitted `live_launch_command` only after the contract passes.
- Intended preflight after a compile succeeds and before a runtime launch:
  1. Copy the compile env log to the runtime host near the planned serve log.
  2. Run `python3 validation_scripts/qwen36_launch_preflight.py --compile-env-log <runtime_compile_env> --serve-log <serve.log> --launch-script tmp_launch_qwen36_segcte2048.sh --run --output-json <launch-preflight.json>`.
  3. Start live vLLM only with the emitted `live_launch_command` if the preflight passes.
...
58. Runtime matrix now enforces log-scan-before-speed.
   - Bug found during source review: `validation_scripts/qwen36_runtime_validation_matrix.py` could run the speed benchmark when the step list omitted `runtime_log_scan` (for example via `--skip-log-scan`) and still report `coherence_and_log_scan_passed=true` as long as no executed coherence step failed. That violates the project rule that runtime logs must be clean before any speed claim.
   - Mitigation: `run_matrix` now counts coherence/log-scan gate steps, requires at least one log-scan step, and skips all speed steps unless every gate step has completed and passed. The skipped speed step records `skip_reason=runtime_log_scan_not_run`, `coherence_or_log_scan_not_completed`, or `coherence_or_log_scan_failed` as appropriate. The summary now includes `gate_counts` so monitor reports can show whether the log scan actually ran.
   - Follow-up: `validation_scripts/qwen36_speed_slice_decision.py` now distinguishes incomplete validation from a real coherence/log failure. If `runtime_validation_summary.json` shows no failed coherence/log rows but the log scan did not run or gates did not complete, the decision is `rerun_runtime_validation` with the specific reason instead of `stop_incoherent`. Actual failed coherence/log rows still produce `stop_incoherent`.
   - Follow-up: the compact `runtime_validation_summary.json` speed-slice annotation now includes `failed_runtime_gate_count` and `next_required_flags` from the full decision JSON. This lets monitors and handoff readers distinguish real failed coherence/log gates from incomplete validation, and see the exact required flags before creating the next compile automation.
   - Follow-up bug found during test review: legacy/partial summaries with `coherence_and_log_scan_passed=false` but no failed coherence/log rows could still fall through to `stop_incoherent`. Mitigation: such ambiguous summaries now return `rerun_runtime_validation` with `reason=coherence_or_log_scan_not_completed`; tests now require an explicit failed coherence/log row before `stop_incoherent`.

59. Compile dry-run source identity is now immutable in the env log.
...
62. Launch preflight wrapper for compile-env-derived runtime launch.
   - Added `validation_scripts/qwen36_launch_preflight.py` to derive the launch environment directly from the compile env log, run `tmp_launch_qwen36_segcte2048.sh` with `LAUNCH_DRY_RUN=1`, audit the generated launch env with `qwen36_launch_env_audit.py`, and emit the `live_launch_command` only after the audit passes.
   - Error encountered during focused local tests: `python3 -m pytest test/unit/scripts/test_qwen36_launch_preflight.py test/unit/scripts/test_qwen36_compile_monitor_prompt.py test/unit/scripts/test_qwen36_validation_tool_manifest.py` failed two CLI tests because `python3 validation_scripts/qwen36_launch_preflight.py ...` raised `ModuleNotFoundError: No module named 'validation_scripts'` before reaching the expected env-key validation. Root cause: executing a script by file path sets `sys.path[0]` to `validation_scripts/`, not the repo root, so the sibling package import was not resolvable. Mitigation: insert the repo root derived from `__file__` into `sys.path` before importing `qwen36_launch_env_audit`.
   - Follow-up mitigation: malformed compile env logs now return structured JSON with `failure_stage=build_preflight` instead of a Python traceback, so heartbeat automations can report the exact missing key.

63. Artifact config policy audit.
   - Extended `validation_scripts/qwen36_artifact_config_audit.py` with `--env-log` policy checks so a completed artifact must prove its `neuron_config.json` matches the compile env before rsync/runtime. It now fails on mismatched `SEQ_LEN`, `MAX_CONTEXT_LENGTH`, CTE/TKG/prefix buckets, context-encoding bucket pairs, GDN cache dtypes, QKV NKI/QK-norm/RoPE/out-proj/KV-quant flags, prefix CTE backend/segment size, sampling mode, output logits, vocab-parallel sampling, and lm_head quant policy.
   - Added the artifact audit script and its unit test to `qwen36_validation_tool_manifest.py`, and updated compile monitor prompts to run `qwen36_artifact_config_audit.py <artifact> --env-log <envlog> --compile-log <compile.log> --recommended-block-size 256 --min-usable-headroom-blocks 0 --no-strict-hybrid-gate` before rsync.
   - Error encountered during local test: `python3 contrib/models/Qwen3.6-27B/test/unit/test_qwen36_artifact_config_audit.py` failed `test_cli_returns_nonzero_for_policy_error` with `FileNotFoundError: .../audit.json`. Root cause: the test read the temp output JSON after `TemporaryDirectory` cleanup. Mitigation: read the JSON inside the temporary-directory context before assertions outside it.

64. Unified compile readiness plus artifact policy.
   - `validation_scripts/qwen36_compile_status.py` now supports `--require-artifact-policy`. In running/incomplete states it reports `artifact_policy.skipped_reason=compile_not_ready`; once basic compile readiness passes, the same status JSON embeds the artifact policy audit and only returns `ready=true` when `artifact_policy.passed=true`.
   - Compile monitor prompts now use `qwen36_compile_status.py --env-log <envlog> --require-artifact-policy --zero-when-running` for polling and require a strict status verdict with `ready=true`, `basic_ready=true`, and `artifact_policy.passed=true` before rsync.

65. Next compile preflight automation payload.
   - Added `validation_scripts/qwen36_next_compile_preflight.py`. It consumes a `speed_slice_decision.json` whose decision is `launch_next_speed_slice`, runs the compile driver once with the structured `COMPILE_DRY_RUN=1` env, reads the generated `ENVLOG`, builds a concrete `codex_app.automation_update` heartbeat-create payload with `qwen36_compile_monitor_prompt.py`, and emits the live compile command only as `compile_command_after_automation`.
   - Updated `validation_scripts/qwen36_speed_slice_decision.py` so `next_preflight.run_order` is now: run `qwen36_next_compile_preflight.py`, create the heartbeat automation from the emitted payload JSON, then run the compile command after the automation exists. The older raw `automation_payload_command_template` remains for compatibility, but it is no longer the primary path.
   - This implements the user rule: compiles should have automation set up first, and we should not continuously poll compile hosts by hand.
   - Verification passed locally: `python3 -m py_compile validation_scripts/qwen36_next_compile_preflight.py validation_scripts/qwen36_speed_slice_decision.py validation_scripts/qwen36_validation_tool_manifest.py test/unit/scripts/test_qwen36_next_compile_preflight.py test/unit/scripts/test_qwen36_speed_slice_decision.py test/unit/scripts/test_qwen36_validation_tool_manifest.py`; focused pytest for next-compile preflight/speed decision/manifest (`22 passed`); full `python3 -m pytest test/unit/scripts` (`98 passed`); Qwen compile-driver unit test (`6 passed`); artifact config audit unit test (`5 passed`); validation-tool manifest build/verify with `file_count=32`, `mismatch_count=0`.

66. Compile monitor prompt now names next-compile preflight helper.
   - Updated `validation_scripts/qwen36_compile_monitor_prompt.py` so future heartbeat monitors no longer describe the old manual "new dry-run env log + prompt generator" path. When `speed_slice_decision.json` says `launch_next_speed_slice`, the monitor prompt now instructs: run `qwen36_next_compile_preflight.py --decision-json ... --output-json ... --automation-json-output ...`, create the fresh heartbeat automation from `<automation-payload.json>` with `codex_app.automation_update`, then release the compile command only through the acknowledgement flow documented in item 67.
   - Verification passed locally: prompt/preflight/speed-decision focused pytest (`24 passed`), full `python3 -m pytest test/unit/scripts` (`98 passed`), Qwen compile-driver unit test (`6 passed`), artifact config audit unit test (`5 passed`), and validation-tool manifest verify with `file_count=32`, `mismatch_count=0`.

67. Compile command release now requires automation acknowledgement.
   - Tightened `validation_scripts/qwen36_next_compile_preflight.py`: the first preflight run now emits the heartbeat automation payload and holds `compile_command_after_automation=None`. The live compile command is released only when the helper is rerun with `--automation-created-name <name>` and that name exactly matches the generated automation payload `name`.
   - Updated `validation_scripts/qwen36_speed_slice_decision.py` to expose a `compile_command_release_command_template` and a four-step run order: run next-compile preflight, create heartbeat automation from payload JSON, rerun next-compile preflight with the created automation name, then run the released compile command.
   - Updated `validation_scripts/qwen36_compile_monitor_prompt.py` so future monitors follow the same two-phase command-release sequence instead of reading a live compile command from the first preflight JSON.
   - Error encountered during local focused tests: `python3 -m pytest test/unit/scripts/test_qwen36_next_compile_preflight.py test/unit/scripts/test_qwen36_speed_slice_decision.py test/unit/scripts/test_qwen36_compile_monitor_prompt.py test/unit/scripts/test_qwen36_runtime_validation_matrix.py` initially failed 5 tests in `test_qwen36_next_compile_preflight.py` with `ValueError: next_preflight.launch_command_after_automation is required`. Context: `qwen36_speed_slice_decision.py` intentionally stopped exposing `launch_command_after_automation` in the first-stage payload. Root cause: `qwen36_next_compile_preflight.py` still read that now-held field instead of reconstructing the command from structured `launch_env` plus `compile_driver`. Fix: added `_quote_env_command()` and rebuilt the live command from `launch_env` only during the acknowledgement/release path. Verification after fix: focused pytest `37 passed`.
   - Full verification after fix: `python3 -m pytest test/unit/scripts` (`101 passed`), Qwen compile-driver unit test (`6 passed`), artifact config audit unit test (`5 passed`), and validation-tool manifest verify with `file_count=32`, `mismatch_count=0`.

68. Compile command release preserves first-preflight context.
   - Tightened the release command emitted by `validation_scripts/qwen36_next_compile_preflight.py`: it now carries the same `--repo-root`, compile/runtime hosts, resolved source dir/commit, generated automation name, heartbeat interval, launch script, and boundary lengths used by the first preflight. The result JSON also includes `release_preflight_context` so a reviewer can see exactly what will be reused when the command is released.
   - Reason: without these arguments, the acknowledgement run could accidentally fall back to local defaults while still checking the automation name, which would weaken the automation-before-compile invariant.
   - Verification passed locally: focused next-preflight/speed-decision/prompt pytest (`27 passed`), full `python3 -m pytest test/unit/scripts` (`101 passed`), Qwen compile-driver unit test (`6 passed`), artifact config audit unit test (`5 passed`), and validation-tool manifest verify with `file_count=32`, `mismatch_count=0`.

69. Compile monitor prompt now uses emitted release template.
   - Updated `validation_scripts/qwen36_compile_monitor_prompt.py` so heartbeat monitors no longer show a minimal acknowledgement command. They now must read `compile_command_release_command_template` and `release_preflight_context` from `<next-compile-preflight.json>` and use the emitted template, replacing only `<NEXT_COMPILE_RELEASE_JSON>`, before running the released `compile_command_after_automation`.
   - Reason: the release template produced by `qwen36_next_compile_preflight.py` carries the exact first-preflight context from item 68; the monitor prompt should not invite a hand-written command that drops that context.
   - Verification passed locally: focused compile-monitor/next-preflight/speed-decision pytest (`27 passed`), full `python3 -m pytest test/unit/scripts` (`101 passed`), Qwen compile-driver unit test (`6 passed`), artifact config audit unit test (`5 passed`), and validation-tool manifest verify with `file_count=32`, `mismatch_count=0`.

70. Runtime coherence defaults include all known short/boundary failure lengths.
   - Expanded the default exact-boundary matrix in `validation_scripts/qwen36_runtime_validation_matrix.py`, `validation_scripts/qwen36_compile_monitor_prompt.py`, and `validation_scripts/qwen36_next_compile_preflight.py` from `146,160,485,505,526,1225,2048,2049,2500,4092,4096` to `123,146,160,485,505,526,1225,1265,1346,2048,2049,2500,4092,4096`.
   - Reason: earlier failures included the short degenerate/logits window around `123` and targeted long-boundary checks around `1265` and `1346`; leaving them out lets an automated compile monitor make an incomplete coherence claim before speed benchmarking.
   - Local patch-context miss encountered while adding tests: the first `apply_patch` expected `test_qwen36_next_compile_preflight.py` assertions adjacent to a blank line that had moved, so it failed with `Failed to find expected lines`. Mitigation: re-read the current section with `sed` and re-applied the test patch against the actual context. No source files were partially modified by that failed patch.
   - Verification passed locally: py_compile for the changed validation scripts/tests; focused pytest for runtime matrix, monitor prompt, and next-compile preflight (`27 passed`); full `python3 -m pytest test/unit/scripts` (`103 passed`); Qwen compile-driver unit test (`6 passed`); artifact config audit unit test (`5 passed`); validation-tool manifest build/verify with `file_count=32`, `mismatch_count=0`.
   - Follow-up: the monitor prompt now passes `--boundary-lengths <known-failure-list>` explicitly to `qwen36_next_compile_preflight.py` when creating the next speed-slice automation. This prevents a monitor running from an older remote checkout from falling back to a narrower preflight default. Verification passed locally after this follow-up: py_compile for the monitor prompt and test; focused prompt/preflight pytest (`16 passed`); full `python3 -m pytest test/unit/scripts` (`103 passed`); Qwen compile-driver unit test (`6 passed`); artifact config audit unit test (`5 passed`); validation-tool manifest build/verify with `file_count=32`, `mismatch_count=0`.
   - Follow-up: before a monitor creates a next speed-slice automation, it must safely sync the remote source checkout by requiring `git status --short` to be empty, then `git fetch origin <branch>` and `git merge --ff-only origin/<branch>`. It must not reset, rebase, stash, or overwrite local changes; dirty trees or fast-forward failures are reported as `source_sync_blocked` and no compile command is created/released. This prevents the next compile from being launched from stale validation/preflight tooling after the current in-flight artifact finishes. Verification passed locally: py_compile for the monitor prompt and test; focused prompt/preflight pytest (`16 passed`); full `python3 -m pytest test/unit/scripts` (`103 passed`); Qwen compile-driver unit test (`6 passed`); artifact config audit unit test (`5 passed`); validation-tool manifest build/verify with `file_count=32`, `mismatch_count=0`.
   - Follow-up: the monitor prompt's preferred runtime-validation command now also passes `--boundary-lengths <known-failure-list>` explicitly. This aligns the generated source prompt with the active automation and prevents the runtime matrix from depending on defaults when a remote checkout is stale or partially synced. Verification passed locally: py_compile for the monitor prompt and test; focused prompt/preflight pytest (`16 passed`); full `python3 -m pytest test/unit/scripts` (`103 passed`); Qwen compile-driver unit test (`6 passed`); artifact config audit unit test (`5 passed`); validation-tool manifest build/verify with `file_count=32`, `mismatch_count=0`.
   - Follow-up: `validation_scripts/qwen36_speed_slice_decision.py` now carries boundary lengths into `next_preflight` and includes `--boundary-lengths <known-failure-list>` in both emitted `next_compile_preflight_command_template` and `compile_command_release_command_template`. `validation_scripts/qwen36_runtime_validation_matrix.py` passes its actual `args.boundary_lengths` into that decision, so the generated decision JSON itself preserves the coherence gate even if a monitor consumer reads command templates directly instead of the prompt prose. Verification passed locally: py_compile for speed-slice decision, runtime matrix, and their tests; focused speed-decision/runtime-matrix/next-preflight pytest (`29 passed`); full `python3 -m pytest test/unit/scripts` (`103 passed`); Qwen compile-driver unit test (`6 passed`); artifact config audit unit test (`5 passed`); validation-tool manifest build/verify with `file_count=32`, `mismatch_count=0`.

71. Compile env source identity now comes from `REPO`.
   - Fixed `tmp_compile_qwen32k_segcte2048_gdnseg512.sh` so `SOURCE_COMMIT` and `SOURCE_BRANCH` are computed with `git -C "${REPO}" ...` instead of plain `git ...` from the caller's current directory.
   - Root cause: next-compile preflight runs the driver from a repo root, but the driver also accepts `REPO=<source checkout>`. If those ever diverged, the env log could claim one source path and another commit. The monitor prompt and artifact audit rely on that env log, so the source identity must be tied to `REPO`.
   - Added a regression test with separate caller and source git repositories; the dry-run env log must report the source repo commit even when the script is launched from the caller repo.
   - Error encountered during local verification: `python3 contrib/models/Qwen3.6-27B/test/unit/test_qwen36_compile_driver.py` initially failed `test_source_commit_comes_from_repo_env_not_caller_cwd` with `AssertionError: 'main' != 'master'`. Root cause: the local git default branch is `main`, while the test hard-coded `master`. Mitigation: the test now reads `git rev-parse --abbrev-ref HEAD` from the initialized source repo and compares against that branch name.
   - Verification passed locally after mitigation: compile-driver unit test (`7 passed`), py_compile for the updated compile-driver test, full `python3 -m pytest test/unit/scripts` (`103 passed`), artifact config audit unit test (`5 passed`), validation-tool manifest build/verify with `file_count=32`, `mismatch_count=0`.

72. Next-compile preflight treats decision boundary lengths as authoritative.
   - Updated `validation_scripts/qwen36_next_compile_preflight.py` so `next_preflight.boundary_lengths` from `speed_slice_decision.json` is the authoritative runtime/next-compile boundary list. If the CLI provides a conflicting non-default `--boundary-lengths`, the helper now fails with `boundary-lengths does not match decision next_preflight` instead of silently narrowing the coherence matrix.
   - Reason: after item 70, boundary lengths are written into the decision JSON. A monitor or operator could still call `qwen36_next_compile_preflight.py` directly with a narrower CLI list; the helper should preserve the decision’s coherence gate by construction.
   - Added tests for decision-boundary preference when the CLI uses defaults and for explicit conflicting boundary rejection.
   - Verification passed locally: py_compile for next-compile preflight and its test; focused next-preflight/speed-decision pytest (`20 passed`); full `python3 -m pytest test/unit/scripts` (`105 passed`); compile-driver unit test (`7 passed`); artifact config audit unit test (`5 passed`); validation-tool manifest build/verify with `file_count=32`, `mismatch_count=0`.

73. Boundary/chat validators reject false coherent text.
   - Hardened `validation_scripts/qwen36_openai_boundary_apc_probe.py`: each boundary row now records `usage_prompt_tokens`, `prompt_tokens_match`, `text_coherence_failure`, and `text_coherence_passed`; the summary fails if any row has status/openai-body failure, prompt-token mismatch, empty text, replacement characters, or the known token-0 exclamation output (`!` / repeated `!`).
   - Hardened `validation_scripts/qwen36_chat_completion_context_bench.py` row gating with the same lightweight text check, so a streamed HTTP 200 with content chunk `!` or empty text no longer counts as a passed chat coherence row.
...
75. Next-slice automation payload template preserves boundary gates.
   - Updated `validation_scripts/qwen36_speed_slice_decision.py` so the legacy `automation_payload_command_template` includes `--boundary-lengths <exact-list>`. The main next-compile preflight path already preserved boundary lengths, but this removes a fallback footgun that could silently drop the known failure windows.
   - Added regression coverage in `test/unit/scripts/test_qwen36_speed_slice_decision.py` for the generated template and in `test/unit/scripts/test_qwen36_next_compile_preflight.py` for the actual automation payload prompt. The next-slice payload must now include the runtime validation checkout sync guard, `validation_source_sync_blocked`, compiled-artifact identity wording, and the exact boundary list.
   - Verification passed locally: py_compile for changed scripts/tests; focused speed-slice/next-preflight pytest (`20 passed`); full `python3 -m pytest test/unit/scripts` (`108 passed`); compile-driver unit test (`7 passed`); artifact config audit unit test (`5 passed`); validation-tool manifest build/verify with `file_count=32`, `mismatch_count=0`.

76. Raw prefill speed gate now rejects mismatched usage prompt tokens.
   - Updated `validation_scripts/qwen36_raw_completion_prefill_bench.py` so every speed row records `prefill_tokens_match_actual`, requires it for `row_gate_passed`, and excludes mismatched rows from mean/min/max prefill speed calculations.
   - Reason: the speed target is usage-accounted, but the benchmark sends exact token-id prompts. If vLLM reports `usage.prompt_tokens` that does not match the actual prompt length, the resulting `usage.prompt_tokens / TTFT` speed is not valid evidence and must not advance a speed slice.
   - Added regression coverage in `test/unit/scripts/test_qwen36_raw_completion_prefill_bench.py` for row-gate rejection and valid-speed exclusion.
   - Verification passed locally: py_compile for changed scripts/tests; focused raw-speed/runtime-matrix/speed-decision pytest (`33 passed`); full `python3 -m pytest test/unit/scripts` (`109 passed`); compile-driver unit test (`7 passed`); artifact config audit unit test (`5 passed`); validation-tool manifest build/verify with `file_count=32`, `mismatch_count=0`.

77. Chat coherence probe now requires usage prompt-token agreement.
   - Updated `validation_scripts/qwen36_chat_completion_context_bench.py` so every chat row records `usage_prompt_tokens` and `prompt_tokens_match`, and `_row_passed` rejects rows where server-reported prompt tokens do not match the locally constructed chat prompt length.
...
80. Runtime matrix cannot silently drop long-context coherence probes.
   - Tightened `validation_scripts/qwen36_runtime_validation_matrix.py`: `--long-lengths` must be non-empty unless the caller explicitly sets `--skip-long-boundary` and provides `--skip-long-boundary-reason`.
...
84. Runtime matrix requires the full coherence contract before speed.
   - Tightened `validation_scripts/qwen36_runtime_validation_matrix.py`: speed-enabled runs now require repeated 2500 with at least 3 repeats, the unique 4k sweep to cover `4088..4104`, long probes to include `8192,16384`, and multi-turn chat to include `160,1225,2500` with at least 8 turns.
   - Reason: the compile monitor prompt already described these as the coherence matrix. This makes the script enforce that contract before any 16k speed claim or next-slice decision.
   - Error encountered during focused local verification: `python3 -m pytest test/unit/scripts/test_qwen36_runtime_validation_matrix.py` failed with `ValueError: --long-lengths is missing required values: 8192,16384` in `test_build_steps_requires_long_boundary_unless_explicitly_skipped`; the test expected the explicit `--skip-long-boundary-reason` error. Root cause: the new generic speed-contract check ran before explicit skip validation. Mitigation: moved `_require_speed_eligible_coherence_contract(args)` after long/chat skip validation. Verification after mitigation: focused runtime matrix pytest (`15 passed`).
   - Added regression coverage in `test/unit/scripts/test_qwen36_runtime_validation_matrix.py`.
   - Verification passed locally: py_compile for changed scripts/tests; focused runtime matrix pytest (`15 passed`); full `python3 -m pytest test/unit/scripts` (`119 passed`); compile-driver unit test (`7 passed`); artifact config audit unit test (`6 passed`); validation-tool manifest build/verify with `file_count=32`, `mismatch_count=0`.

85. Runtime matrix enforces the exact 16k cold-prefill speed target.
   - Tightened `validation_scripts/qwen36_runtime_validation_matrix.py`: speed-enabled runs must use exactly `--speed-lengths 16384`, at least 3 repeats, `--speed-max-tokens 1`, and `--min-prefill-tok-s >= 3000`.
   - Reason: `qwen36_raw_completion_prefill_bench.py` reports one aggregate mean over all requested lengths. Allowing easier extra lengths such as `8192,16384` could mask a slow 16k row, so the matrix now treats the speed gate as exactly the intended 16k cold-prefill target.
   - Added regression coverage in `test/unit/scripts/test_qwen36_runtime_validation_matrix.py`.
   - Verification passed locally: py_compile for changed scripts/tests; focused runtime matrix pytest (`16 passed`); full `python3 -m pytest test/unit/scripts` (`120 passed`); compile-driver unit test (`7 passed`); artifact config audit unit test (`6 passed`); validation-tool manifest build/verify with `file_count=32`, `mismatch_count=0`.

86. Speed-slice decisions require a runtime validation contract.
   - `validation_scripts/qwen36_runtime_validation_matrix.py` now writes `validation_contract` into `runtime_validation_summary.json`, capturing the exact coherence/speed contract that was enforced.
   - `validation_scripts/qwen36_speed_slice_decision.py` now refuses to mark a candidate or launch the next speed slice unless the summary has `schema=qwen36-runtime-validation-contract-v1`, is speed-eligible, and proves the full boundary, repeated, 4k sweep, long, chat, and exact 16k speed gates. The summary contract's boundary list is authoritative for generated next-compile automation payloads.
   - Error encountered during focused local verification: `python3 -m pytest test/unit/scripts/test_qwen36_runtime_validation_matrix.py test/unit/scripts/test_qwen36_speed_slice_decision.py` initially failed because `min_prefill_tok_s=2999.0` did not raise. Root cause: the `min_prefill_tok_s` check was accidentally indented under the `speed_max_tokens` error branch. Mitigation: moved it into the main speed-contract path; focused tests then passed.
   - Error encountered during full local verification: `python3 -m pytest test/unit/scripts` initially failed 7 `test_qwen36_next_compile_preflight.py` cases with `decision JSON is not requesting a compile; decision='rerun_runtime_validation'`. Root cause: next-preflight fixtures generated decisions from hand-written runtime summaries without `validation_contract`. Mitigation: added valid contract fixtures and updated normal preflight assertions to expect the full contract boundary list, leaving only the conflict test narrowed.
   - Verification passed locally after mitigation: py_compile for changed scripts/tests; focused runtime-matrix/speed-decision/next-preflight pytest (`38 passed`); full `python3 -m pytest test/unit/scripts` (`122 passed`); compile-driver unit test (`7 passed`); artifact config audit unit test (`6 passed`); validation-tool manifest build/verify with `file_count=32`, `mismatch_count=0`.

87. Speed-slice decisions require the raw 16k usage-accounted speed output contract.
   - `validation_scripts/qwen36_speed_slice_decision.py` now validates the attached `raw_prefill_speed.json` before it can mark a candidate, launch the next speed slice, or profile a slow coherent artifact.
   - Required raw-speed evidence: `lengths == [16384]`, `repeats >= 3`, `max_tokens == 1`, `allow_usage_fallback == false`, `require_text == false`, `min_prefill_tok_s >= 3000`, `prefill_tokens_all_match_actual == true`, `speed_gate.enabled == true`, and `speed_gate.min_prefill_tok_s >= 3000`.
   - If the raw speed output violates any of these fields, the decision is `rerun_speed_validation` with `reason=speed_output_contract_mismatch` and `speed_output_contract_errors=[...]`. This prevents stale 8k runs, mixed-length aggregate means, tokenizer fallback, missing prompt-token accounting, or disabled/weak speed gates from driving another compile automation.
   - This preserves the current workflow rule: compile hosts are monitored by heartbeat automation; do not manually poll long compiles unless the user asks or the automation wakes this thread.
   - Verification passed locally: py_compile for speed-decision/runtime-matrix/next-preflight scripts and tests; focused speed-decision/runtime-matrix/next-preflight pytest (`39 passed`); raw-prefill benchmark pytest (`10 passed`); full `python3 -m pytest test/unit/scripts` (`123 passed`); compile-driver unit test (`7 passed`); artifact config audit unit test (`6 passed`); validation-tool manifest build/verify with `file_count=32`, `mismatch_count=0`.

88. Speed-slice decisions require complete raw speed rows, not just top-level settings.
   - `validation_scripts/qwen36_speed_slice_decision.py` now validates the `raw_prefill_speed.json` `results` array before any candidate/next-slice/profile decision.
   - Required row evidence: exactly one 16k length times the declared repeat count, repeat indexes `0..repeats-1`, `target_prompt_tokens == actual_prompt_tokens == prefill_tokens == usage.prompt_tokens == 16384`, `prefill_token_source == "usage"`, `prefill_tokens_match_actual == true`, HTTP status below 400, positive TTFT, and positive per-row prefill tok/s.
   - If the raw speed JSON is missing/truncated/malformed, or if any row used fallback/mismatched prompt accounting, the decision is `rerun_speed_validation` with `reason=speed_output_contract_mismatch` and row-specific `speed_output_contract_errors`.
   - Reason: a top-level `lengths=[16384]` and passing aggregate gate is not enough if the JSON was partial, stale, or manually assembled. The automation path must only advance from the real three-repeat usage-accounted 16k cold-prefill benchmark.
   - Verification passed locally: py_compile for speed-decision/runtime-matrix/next-preflight scripts and tests; focused speed-decision/runtime-matrix/next-preflight pytest (`40 passed`); full `python3 -m pytest test/unit/scripts` (`124 passed`); compile-driver unit test (`7 passed`); artifact config audit unit test (`6 passed`); validation-tool manifest build/verify with `file_count=32`, `mismatch_count=0`.

89. Profile comparison rejects stale or weak speed JSON.
   - `validation_scripts/qwen36_profile_summary_compare.py` now validates `--speed-json` when building the profile report. The speed JSON must match the report `--prompt-tokens`, use usage-accounted rows, have at least three repeats, and carry per-row prompt-token agreement/status/TTFT/speed evidence.
   - Reason: `profile_slow_coherent` is the next action if all planned coherent speed slices miss the 3k tok/s target. The profile diagnosis should not be compared against stale 8k speed data, a one-repeat diagnostic run, or tokenizer-fallback speed accounting.
   - `extract_speed_summary()` remains usable for ad hoc parsing without strict checks, but `build_report(..., speed_json=...)` enforces the contract because that is the automation/profile path.
   - Verification passed locally: py_compile for profile summary compare and tests; focused profile/speed-decision pytest (`23 passed`); full `python3 -m pytest test/unit/scripts` (`126 passed`); compile-driver unit test (`7 passed`); artifact config audit unit test (`6 passed`); validation-tool manifest build/verify with `file_count=32`, `mismatch_count=0`.

90. Compile monitor prompts now make every speed-slice decision branch explicit.
   - `validation_scripts/qwen36_compile_monitor_prompt.py` now tells heartbeat monitors how to handle `stop_incoherent`, `rerun_runtime_validation`, `rerun_speed_validation`, `candidate_meets_target`, `launch_next_speed_slice`, and `profile_slow_coherent`.
   - `rerun_speed_validation` is explicitly limited to rerunning the exact 16k usage-accounted speed validation after coherence/log-scan passed; it must not launch another compile from `speed_output_contract_mismatch`, `speed_rows_failed_before_speed_gate`, or `no_valid_prefill_speed`.
   - `candidate_meets_target` now instructs the monitor to report the coherent-speed artifact and delete the completed automation. `profile_slow_coherent` now states that `qwen36_profile_summary_compare.py` enforces the same 16k speed JSON contract and that rejected speed JSON requires rerunning exact speed validation before profile conclusions.
   - Reason: the scripts already enforce these gates, but the automation prompt should not leave room for a heartbeat to improvise around a failed validation or weak speed output.
   - Verification passed locally: py_compile for compile-monitor prompt and test; focused compile-monitor/speed-decision pytest (`26 passed`); full `python3 -m pytest test/unit/scripts` (`126 passed`); compile-driver unit test (`7 passed`); artifact config audit unit test (`6 passed`); validation-tool manifest build/verify with `file_count=32`, `mismatch_count=0`.

91. Active sample-token compile automation updated in place.
   - Updated the existing heartbeat automation `monitor-qwen-sampletokonly-compile` to include the stricter decision handling from item 90 without changing its 30-minute schedule.
   - The updated automation still monitors the in-flight sample-token CTE2048 artifact from source commit `0901798`, but it now requires the runtime/compile checkouts to fast-forward to `origin/codex/qwen36-prefill-speed-coherent` before runtime validation or any next compile. This lets future wakeups pick up commits through `ce414a2` while preserving the compiled artifact identity in the copied env log.
   - The automation prompt now explicitly blocks launching another compile from `rerun_runtime_validation`, `rerun_speed_validation`, weak speed JSON, or incoherent runtime validation; it only releases a next compile after creating the next heartbeat automation.

92. Runtime summary preserves speed-decision failure evidence.
   - `validation_scripts/qwen36_runtime_validation_matrix.py` now copies `speed_json`, `speed_gate`, `prefill_tok_s_mean`, and `speed_output_contract_errors` from `speed_slice_decision.json` into the compact `runtime_validation_summary.json` annotation.
   - Reason: the standalone decision JSON remains authoritative, but monitors and handoff readers often inspect the runtime summary first. If a candidate returns `rerun_speed_validation` because the raw speed JSON is stale, malformed, or missing 16k usage-accounted rows, that exact error list must not be hidden in the compact summary.
   - Added regression coverage that forces `speed_output_contract_mismatch` from an empty raw-speed `results` array and verifies the compact runtime summary includes `results_count`, the raw speed path, speed gate, and mean speed.
   - Verification passed locally: py_compile for runtime matrix and test; focused runtime-matrix/speed-decision pytest (`33 passed`); full `python3 -m pytest test/unit/scripts` (`127 passed`); compile-driver unit test (`7 passed`); artifact config audit unit test (`6 passed`); validation-tool manifest build/verify with `file_count=32`, `mismatch_count=0`.

93. Runtime validation summary now records validator/tool provenance.
   - `validation_scripts/qwen36_runtime_validation_matrix.py` now builds `<output-dir>/validation_tool_manifest.json` by default and annotates `runtime_validation_summary.json` with `validation_tool_manifest_path` and compact manifest status (`schema`, `git_commit`, `file_count`, `missing`, `passed`).
   - The monitor prompt now requires `validation_tool_manifest.passed=true` in the runtime summary and treats missing/false manifest provenance as a validation failure, not a reason to advance to another compile or speed claim.
   - Reason: coherent-speed decisions depend on the hardened validators and gates. Runtime summaries should prove which validation scripts produced the verdict instead of relying only on branch names or operator memory.
   - Added regression coverage for direct manifest attachment with test files included.
   - Verification passed locally: py_compile for runtime matrix/monitor prompt/tests; focused runtime-matrix/compile-monitor/manifest pytest (`35 passed`); full `python3 -m pytest test/unit/scripts` (`128 passed`); compile-driver unit test (`7 passed`); artifact config audit unit test (`6 passed`); validation-tool manifest build/verify with `file_count=32`, `mismatch_count=0`.

94. Speed-slice decisions now require validator provenance before speed/next-compile actions.
   - `validation_scripts/qwen36_speed_slice_decision.py` now rejects runtime summaries missing `validation_tool_manifest`, carrying the wrong manifest schema, or reporting `validation_tool_manifest.passed=false`. The decision becomes `rerun_runtime_validation` with reason `missing_validation_tool_manifest`, `validation_tool_manifest_schema_mismatch`, or `validation_tool_manifest_failed`.
   - Reason: item 93 made validator provenance visible, but the decision gate still only checked the runtime contract and speed JSON. A summary with failed/missing validation-tool provenance must not produce `candidate_meets_target`, `launch_next_speed_slice`, or `profile_slow_coherent`.
   - Error encountered during focused local verification: `python3 -m pytest test/unit/scripts/test_qwen36_speed_slice_decision.py test/unit/scripts/test_qwen36_runtime_validation_matrix.py test/unit/scripts/test_qwen36_next_compile_preflight.py` initially failed 10 tests. Representative failures: `AssertionError: assert 'rerun_runtime_validation' == 'launch_next_speed_slice'` in `test_attach_speed_slice_decision_writes_next_action`, `AssertionError: assert 'rerun_runtime_validation' == 'profile_slow_coherent'` in `test_attach_speed_slice_decision_surfaces_profile_preflight`, and `ValueError: decision JSON is not requesting a compile; decision='rerun_runtime_validation'` in next-compile preflight tests. Root cause: hand-written runtime-summary fixtures in runtime-matrix and next-preflight tests did not include the new `validation_tool_manifest` compact annotation, so the new gate correctly reran runtime validation. Mitigation: added valid manifest fixtures to those summaries and added explicit missing/schema-failed manifest regression tests.
   - Verification passed locally after mitigation: py_compile for speed-decision/runtime-matrix/next-preflight scripts and tests; focused speed-decision/runtime-matrix/next-preflight pytest (`43 passed`); full `python3 -m pytest test/unit/scripts` (`129 passed`); compile-driver unit test (`7 passed`); artifact config audit unit test (`6 passed`); validation-tool manifest build/verify with `file_count=32`, `mismatch_count=0`.

95. Active sample-token automation refreshed for validator-provenance gate.
   - Updated the existing heartbeat automation `monitor-qwen-sampletokonly-compile` in place again, without changing its 30-minute interval.
   - The refreshed prompt now requires `validation_tool_manifest.passed=true`, names the `missing_validation_tool_manifest`, `validation_tool_manifest_schema_mismatch`, and `validation_tool_manifest_failed` decision reasons, and blocks speed/profile/next-compile actions when validator provenance is missing or failed.
   - Local automation metadata check after update confirmed the prompt contains `validation_tool_manifest.passed=true`, `missing_validation_tool_manifest`, `validation_tool_manifest_failed`, and `validator provenance failure`, with `FREQ=MINUTELY;INTERVAL=30` still present.
   - Refreshed the automation again after the explicit operator instruction to set up compile automation rather than continuously monitoring manually. The automation prompt is now the owner for compile completion, EC2-to-EC2 transfer, launch, runtime validation, exact 16k speed validation, and next-slice decisions.

96. Next compile command release now requires the Codex automation id.
   - Tightened `validation_scripts/qwen36_next_compile_preflight.py`: the live `COMPILE_DRY_RUN=0` command is released only after the caller provides both `--automation-created-name` matching the generated payload name and non-empty `--automation-created-id` from the `codex_app.automation_update` result. Supplying only the name now fails with `automation release requires both --automation-created-name and --automation-created-id from codex_app.automation_update`.
   - Updated `validation_scripts/qwen36_compile_monitor_prompt.py` and `validation_scripts/qwen36_speed_slice_decision.py` so future monitor prompts and decision JSON templates explicitly require the returned `automationId` before releasing any next compile.
   - Reason: the previous guard prevented most accidental launches, but a matching name alone was still weaker than the user's "automation for compiles first" rule. The acknowledgement now has to include concrete app-create output.
   - Local search errors encountered and logged during this change: `rg: scripts: No such file or directory (os error 2)` from a repo search that included a nonexistent `scripts` path, and `rg: the literal "\n" is not allowed in a regex` from a malformed call-site search. Both were local inspection mistakes only; mitigation was to rerun searches against existing paths/simple patterns, with no artifact/runtime impact.
   - Verification passed locally: py_compile for changed validation scripts and tests; focused pytest for next-compile preflight, compile-monitor prompt, and speed-slice decision (`36 passed`); full `python3 -m pytest test/unit/scripts` (`130 passed`); compile-driver plus artifact-audit pytest (`13 passed`); validation-tool manifest build/verify with `file_count=32`, `mismatch_count=0`.

97. Active sample-token automation now carries the automation-id release rule.
   - Updated the existing app heartbeat automation `monitor-qwen-sampletokonly-compile` in place after commit `ac5d225`, keeping its 30-minute cadence and the same sample-token artifact paths.
   - The live prompt now says the compiled artifact identity remains source commit `0901798`, while validation and next-slice preflight must fast-forward clean source checkouts to `origin/codex/qwen36-prefill-speed-coherent` so they pick up the validator/automation contract through `ac5d225`.
   - The live prompt now explicitly requires `qwen36_next_compile_preflight.py` to be rerun with both `--automation-created-name` and `--automation-created-id` from `codex_app.automation_update` before any `compile_command_after_automation` is accepted.
   - No compile-host polling was performed for this update; the in-flight compile remains automation-owned.

98. Artifact readiness now enforces the coherent-speed slice anchor.
   - Tightened `validation_scripts/qwen36_artifact_config_audit.py`: when a compile env log carries `SPEED_SLICE=sampletokonly`, `hostlogits`, or `hostlogits_lmheadbf16`, the audit now requires the env log itself to match the safe coherent-speed anchor before policy can pass.
   - The enforced anchor mirrors the compile-driver/speed-decision policy: QKV NKI on, QK-norm fuse on, RoPE fuse off, output-proj NKI off, KV BF16, attention_cte, GDN segment 0, multihead DeltaNet CTE off, direct scan0, BF16 GDN caches, CTE2048, 32k context, and FP8 linear-attention gates. Per-slice sampling/lm-head requirements are checked separately.
...
100. Fixed speed-slice artifact policy to use the env-log CTE field that actually exists.
   - Error found during local validator audit: item 98's new speed-slice policy required `CTE_BUCKETS_RAW=2048`, but `tmp_compile_qwen32k_segcte2048_gdnseg512.sh` writes `CTE_BUCKETS=2048` to the env log and does not write `CTE_BUCKETS_RAW`. Evidence: current script writes `CTE_BUCKETS=${CTE_BUCKETS[*]}` in the env log; `git show 0901798:tmp_compile_qwen32k_segcte2048_gdnseg512.sh` also showed the in-flight compile-era script did not write `CTE_BUCKETS_RAW`.
   - Impact/hypothesis: a valid active sample-token artifact could have failed `qwen36_compile_status.py --require-artifact-policy` with a false `speed_slice_cte_buckets_raw_mismatch`-style error before rsync/launch, because the validator was checking a compile-driver input variable rather than recorded artifact provenance.
...
102. Artifact audit now infers speed slices from legacy active-compile env logs.
   - Error/risk found during local compatibility audit: the active sample-token artifact was compiled from source commit `0901798`, whose compile driver env log did not write `SPEED_SLICE` or `DISABLE_ON_DEVICE_SAMPLING`. Evidence from `git show 0901798:tmp_compile_qwen32k_segcte2048_gdnseg512.sh`: it wrote `BASE`, `SAMPLING=on_device_greedy_${SAMPLING_TAG}`, `OUTPUT_LOGITS_WITH_ON_DEVICE_SAMPLING`, and `CTE_BUCKETS`, but not those newer fields.
   - Impact/hypothesis: item 100 fixed the CTE field, but the speed-slice policy could still silently skip the active artifact because `SPEED_SLICE` was absent. That would allow a `sampletokonly` artifact name/env to pass only generic config-vs-env checks instead of the intended coherent-speed anchor.
   - Fix: `validation_scripts/qwen36_artifact_config_audit.py` now infers `sampletokonly`, `hostlogits`, and `hostlogits_lmheadbf16` from legacy `BASE`/`SAMPLING` fields when `SPEED_SLICE` is absent, and infers `DISABLE_ON_DEVICE_SAMPLING=0` from legacy `SAMPLING=on_device_*`.
   - Added regression coverage using the active compile-era shape: no `SPEED_SLICE`, no `DISABLE_ON_DEVICE_SAMPLING`, `BASE=...sampletokonly...`, and `SAMPLING=on_device_greedy_sampletokonly`; the artifact policy still enforces and passes the sample-token anchor.
   - Verification passed locally: py_compile for audit/status scripts and tests; focused audit/status pytest (`21 passed`); full `python3 -m pytest test/unit/scripts` (`130 passed`); compile-driver plus artifact-audit pytest (`16 passed`); validation-tool manifest build/verify with `file_count=32`, `mismatch_count=0`.

103. Active sample-token automation now requires legacy env-log speed-slice inference.
   - Updated the existing app heartbeat automation `monitor-qwen-sampletokonly-compile` in place after commit `2d39a15`, preserving its 30-minute cadence and same sample-token artifact paths.
