...
55. Runtime verdict to next speed-slice decision helper.
   ...
   - Follow-up fix: generated heartbeat monitor names now include the pinned `TS`, both in `qwen36_compile_monitor_prompt.py` defaults and in the `next_preflight` automation payload template. This prevents repeated `hostlogits` or `hostlogits_lmheadbf16` attempts from all using the same generic monitor name.
   - Future compile monitor prompts now call this helper after runtime validation and explicitly forbid launching the next compile from the current monitor; the next compile still needs its own dry-run env log and heartbeat automation.
   ...
   - Verification passed locally after pinning `TS`: `bash -n tmp_compile_qwen32k_segcte2048_gdnseg512.sh`; `python3 -m py_compile validation_scripts/qwen36_speed_slice_decision.py test/unit/scripts/test_qwen36_speed_slice_decision.py validation_scripts/qwen36_compile_monitor_prompt.py test/unit/scripts/test_qwen36_compile_monitor_prompt.py contrib/models/Qwen3.6-27B/test/unit/test_qwen36_compile_driver.py`; focused pytest for decision+monitor prompt (`14 passed`); direct compile-driver unittest (`6 tests`); dry-run smoke with `TS=20260606T010203Z_probe` plus env-log `TS` check; `python3 -m pytest test/unit/scripts` (`67 passed`); manifest build for 12 files; and manifest verify with `mismatch_count=0`.
