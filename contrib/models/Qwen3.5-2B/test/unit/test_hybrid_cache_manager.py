...
class TestHybridDeltaNetCacheManager(unittest.TestCase):
    def test_allocates_per_layer_cache_shapes(self):
        ...
        self.assertEqual(list(mgr.past_key_values[0].shape), [2, 16, 128, 128])
        self.assertEqual(list(mgr.past_key_values[1].shape), [2, 6144, 3])
    ...
    def test_get_cache_slices_only_full_attention_layers(self):
        ...
        self.assertEqual(list(recurrent_state.shape), [1, 16, 128, 128])
        self.assertEqual(list(conv_state.shape), [1, 6144, 3])
    ...
    def test_deltanet_update_scatters_by_seq_id(self):
        config = _make_config()
        mgr = HybridDeltaNetCacheManager(config, num_kv_head=config.num_key_value_heads)
        recurrent = torch.ones((1, 16, 128, 128), dtype=torch.bfloat16)
        conv = torch.ones((1, 6144, 3), dtype=torch.bfloat16)
    ...
    def test_deltanet_full_batch_update_replaces_state_cache(self):
        config = _make_config()
        mgr = HybridDeltaNetCacheManager(config, num_kv_head=config.num_key_value_heads)
        recurrent = torch.ones((2, 16, 128, 128), dtype=torch.bfloat16)
        conv = torch.ones((2, 6144, 3), dtype=torch.bfloat16)
        recurrent[0].fill_(3)
        recurrent[1].fill_(5)
        conv[0].fill_(11)
        conv[1].fill_(13)
    ...
    def test_get_cache_trims_padding_row_without_seq_ids(self):
        ...
        self.assertEqual(list(recurrent_state.shape), [2, 16, 128, 128])
        self.assertEqual(list(conv_state.shape), [2, 6144, 3])
