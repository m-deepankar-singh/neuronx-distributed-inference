...
The ``--enable-kv-cache-quant`` flag is an explicit long-context recovery
variant for shapes where the BF16 paged-attention cache exceeds device HBM.
"""
...
def _build_config(args: argparse.Namespace):
    ...
    if args.enable_kv_cache_quant:
        ...
        neuron_config_kwargs["kv_quant_config"] = KVQuantizationConfig(
            quant_dtype=to_torch_dtype(args.kv_quant_dtype),
            direct_cast=args.kv_direct_cast,
        )
    else:
        neuron_config_kwargs["kv_cache_quant"] = False
...
def main() -> int:
    ...
    parser.add_argument("--enable-kv-cache-quant", action="store_true")
    parser.add_argument("--kv-quant-dtype", default="float8_e4m3fn")
    parser.add_argument(
        "--kv-direct-cast", dest="kv_direct_cast", action="store_true", default=True
    )
    parser.add_argument("--no-kv-direct-cast", dest="kv_direct_cast", action="store_false")
    ...
    print(
        ...
        json.dumps(
            {
                "enabled": args.enable_kv_cache_quant,
                "quant_dtype": args.kv_quant_dtype,
                "direct_cast": args.kv_direct_cast,
            },
            sort_keys=True,
        ),
        ...
        json.dumps(
            {
                ...
                "kv_cache_quant": args.enable_kv_cache_quant,
                "kv_quant_dtype": args.kv_quant_dtype if args.enable_kv_cache_quant else None,
            },
            sort_keys=True,
        ),
        flush=True,
