...
Mathematical framework (same as nki_deltanet_chunked.py):
  Per-chunk stable triangular solve for intra-chunk correction:
    A = -QK_decay * lower_mask
    N = inv(I - A), solved by two 64x64 forward-substitution blocks plus merge
    value_corr = N @ v_beta
...
def deltanet_fused_chunked_fwd(
    ...
    state_in: nl.ndarray,  # (128, 128) float32 — recurrent state at chunk start
    ...
    # For whole-prompt CTE the caller passes zeros. For vLLM/NxDI chunked
    # prefill, this carries the GDN state from the previous CTE bucket.
    # ================================================================
    ...
    for i_chunk in nl.sequential_range(num_chunks):
        ...
        # Build pairwise decays as exp(gc[i] - gc[j]) so no individual
        # exp(-gc[j]) term can overflow.
        ...
        gc_pair_padded = nl.ndarray((P_MAX, P_MAX), dtype=nl.float32, buffer=nl.sbuf)
        nisa.memset(dst=gc_pair_padded, value=0.0)
        nisa.tensor_copy(dst=gc_pair_padded[0:P_MAX, 0:1], src=gc_p[0:P_MAX, 0:1])

        gc_row_psum2 = nl.ndarray((P_MAX, P_MAX), dtype=nl.float32, buffer=nl.psum)
        nisa.nc_transpose(dst=gc_row_psum2, data=gc_pair_padded)

        gc_row2 = nl.ndarray((1, P_MAX), dtype=nl.float32, buffer=nl.sbuf)
        nisa.tensor_copy(dst=gc_row2[0:1, 0:P_MAX], src=gc_row_psum2[0:1, 0:P_MAX])

        gc_row_broadcast = nl.ndarray((P_MAX, P_MAX), dtype=nl.float32, buffer=nl.sbuf)
        for i_shuf in nl.static_range(P_MAX // 32):
            nisa.nc_stream_shuffle(
                src=gc_row2[0:1, 0:P_MAX],
        ...
        # row-by-row, then merge the lower-left block. This is equivalent to
        # the nilpotent Neumann series but avoids repeated squaring of A.
        # ============================================================
        P_acc = nl.ndarray((P_MAX, P_MAX), dtype=nl.float32, buffer=nl.sbuf)
        nisa.memset(dst=P_acc, value=0.0)

        A_T_psum2 = nl.ndarray((P_MAX, P_MAX), dtype=nl.float32, buffer=nl.psum)
        nisa.nc_transpose(dst=A_T_psum2, data=A_mat)
        A_T2 = nl.ndarray((P_MAX, P_MAX), dtype=nl.float32, buffer=nl.sbuf)
        nisa.tensor_copy(dst=A_T2, src=A_T_psum2)
        ...
        for solve_i in nl.static_range(64):
            ...
            nisa.nc_matmul(dst=row_psum, stationary=A_T2, moving=P_acc)
...
@nki.jit
def deltanet_fused_chunked_fwd_headgroup(
    query: nl.ndarray,  # (HG, S, 128) float32 — l2-normed and scaled
    key: nl.ndarray,  # (HG, S, 128) float32 — l2-normed
    value: nl.ndarray,  # (HG, S, 128) float32
    g_in: nl.ndarray,  # (HG, S, 1) float32 — per-token log-decay
    beta_in: nl.ndarray,  # (HG, S, 1) float32 — per-token write gate
    state_in: nl.ndarray,  # (HG, 128, 128) float32 — recurrent state
    lower_mask: nl.ndarray,  # (128, 128) float32 — strict lower tri
    identity: nl.ndarray,  # (128, 128) float32 — identity
    lower_mask_diag: nl.ndarray,  # (128, 128) float32 — lower tri with diag
):
    """Grouped-head fused DeltaNet forward.

    This is intentionally a conservative megakernel: it moves the local-head
    loop inside NKI while preserving the validated per-head, per-chunk math in
    ``deltanet_fused_chunked_fwd``. It reduces framework/NKI launch overhead
    without changing the recurrence, triangular solve, or state update.
    """
    head_group = query.shape[0]
    seq_len = query.shape[1]
    dim = query.shape[2]  # 128
    num_chunks = seq_len // CHUNK_SIZE

    # Output tensors in HBM
    output = nl.ndarray((head_group, seq_len, dim), dtype=query.dtype, buffer=nl.shared_hbm)
    final_state_out = nl.ndarray(
        (head_group, P_MAX, dim), dtype=nl.float32, buffer=nl.shared_hbm
    )

    # ================================================================
    # Load constant masks into SBUF once (reused across all heads/chunks)
    # ================================================================
    eye = nl.ndarray((P_MAX, P_MAX), dtype=nl.float32, buffer=nl.sbuf)
    nisa.dma_copy(dst=eye, src=identity)
    ...
    for i_head in nl.sequential_range(head_group):
        # ============================================================
        # Initialize recurrent state in SBUF for this head.
        # ============================================================
        state = nl.ndarray((P_MAX, dim), dtype=nl.float32, buffer=nl.sbuf)
        nisa.dma_copy(dst=state, src=state_in[i_head, 0:P_MAX, 0:dim])
        ...
        for i_chunk in nl.sequential_range(num_chunks):
            ...
            nisa.dma_copy(
                dst=q_c,
                src=query[i_head, chunk_start : chunk_start + CHUNK_SIZE, 0:dim],
            )

            k_c = nl.ndarray((P_MAX, dim), dtype=key.dtype, buffer=nl.sbuf)
            nisa.dma_copy(
                dst=k_c,
                src=key[i_head, chunk_start : chunk_start + CHUNK_SIZE, 0:dim],
            )

            v_c = nl.ndarray((P_MAX, dim), dtype=value.dtype, buffer=nl.sbuf)
            nisa.dma_copy(
                dst=v_c,
                src=value[i_head, chunk_start : chunk_start + CHUNK_SIZE, 0:dim],
            )
            ...
            nisa.dma_copy(
                dst=g_chunk_p[0:CHUNK_SIZE, 0:1],
                src=g_in[i_head, chunk_start : chunk_start + CHUNK_SIZE, 0:1],
            )
            ...
            nisa.dma_copy(
                dst=beta_p[0:CHUNK_SIZE, 0:1],
                src=beta_in[i_head, chunk_start : chunk_start + CHUNK_SIZE, 0:1],
            )
            ...
            # Store output chunk to HBM
            nisa.dma_copy(
                dst=output[i_head, chunk_start : chunk_start + CHUNK_SIZE, 0:dim],
                src=chunk_out,
            )
        ...
        # ---- Write final state to HBM ----
        nisa.dma_copy(dst=final_state_out[i_head, 0:P_MAX, 0:dim], src=state)
