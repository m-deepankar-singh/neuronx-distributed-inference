...
def deltanet_chunk_step(
    ...
    # ============================================================
    # Apply N to a single RHS:
    #
    #   baseline: v_new = (N @ v_beta) - (N @ (k_beta * exp_gc)) @ state
    #   v2:       v_new = N @ (v_beta - ((k_beta * exp_gc) @ state))
    #
    # This preserves the materialized-N triangular solve and its layout while
    # removing one N @ RHS application from the hot path.
    # ============================================================
    ...
    nisa.nc_matmul(dst=kbe_T_psum, stationary=kb_exp_gc, moving=eye)
    ...
    vnew_psum = nl.ndarray((P_MAX, dim), dtype=nl.float32, buffer=nl.psum)
    nisa.nc_matmul(dst=vnew_psum, stationary=N_T, moving=solve_rhs)
    v_new = nl.ndarray((P_MAX, dim), dtype=nl.float32, buffer=nl.sbuf)
    nisa.tensor_copy(dst=v_new, src=vnew_psum)
