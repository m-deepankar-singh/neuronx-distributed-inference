...
After the runtime matrix writes `runtime_validation_summary.json`, read `<validation-output-dir>/speed_slice_decision.json` and report its `decision`. If it says `launch_next_speed_slice`, do not launch that compile directly from this monitor. First run `validation_scripts/qwen36_next_compile_preflight.py --decision-json <validation-output-dir>/speed_slice_decision.json --output-json <next-compile-preflight.json> --automation-json-output <automation-payload.json>`. Create a fresh heartbeat automation from `<automation-payload.json>` with `codex_app.automation_update`; only after that automation creation succeeds may the `compile_command_after_automation` from `<next-compile-preflight.json>` be run. If it says `profile_slow_coherent`, follow the emitted `profile_preflight`: stop vLLM or use idle Trainium cores, run `validation_scripts/qwen36_context_neff_profile.py` against loose `context_encoding_model/_tp*_bk*/graph.neff` files with `neuron-explorer capture --io-from=neff`, then compare `summary-json` output with `validation_scripts/qwen36_profile_summary_compare.py`. Do not enable `NEURON_RT_INSPECT_DEVICE_PROFILE=1` on the live vLLM request path; that path has already stalled full-server requests and does not isolate the slow context graph.
