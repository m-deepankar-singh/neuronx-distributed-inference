...
Before the live vLLM launch on {runtime_host}, run `{launch_script}` once with `LAUNCH_DRY_RUN=1` using the artifact and settings derived from the copied compile env log: `MAX_MODEL_LEN=MAX_CONTEXT_LENGTH`, `SEQ_LEN=SEQ_LEN`, `CTE_BUCKETS=CTE_BUCKETS`, `CONTEXT_ENCODING_BUCKET_PAIRS=<compiled pairs plus only optional prefix-0 pairs>`, `TOKEN_GENERATION_BUCKETS=TOKEN_GENERATION_BUCKETS`, `MAX_GDN_CHECKPOINT_SLOTS`, `GDN_RECURRENT_CACHE_DTYPE`, `GDN_CONV_CACHE_DTYPE`, `QWEN36_DELTANET_MULTIHEAD_CTE`, `QWEN36_SPLIT_QKV_TKG_ROW_SCALES`, `QWEN36_DELTANET_FUSED_SEGMENT_TOKENS`, `QWEN36_DELTANET_SOLVE_MODE`, and `QWEN36_DELTANET_SOLVE_SCAN_STEPS`. Then run `validation_scripts/qwen36_launch_env_audit.py --compile-env-log <runtime-compile-env-log> --launch-env-log <launch-env-log> --require-launch-dry-run`; only launch live vLLM after that audit passes. A launch audit failure is a validation failure, not a reason to start vLLM.
